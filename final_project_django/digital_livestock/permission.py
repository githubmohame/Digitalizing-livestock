...
class CustomerAccessPermission(permissions.BasePermission):
	...
	def has_permission(self, request, view):
		...
		if(type(request.user)!=AnonymousUser and request.user.groups.all().filter(name="admin").count()>0 and view.__class__.__name__ in ["modified_gavernorate","modified_city","modified_village","modified_species","modified_platoon" ,"governorate_api","get_locations_api","city_api","village_api","admin_api"]):
			...
			return True
		elif(type(request.user)!=AnonymousUser and request.user.groups.all().filter(name="supervisor").count()>0  and view.__class__.__name__ in["governorate_api","city_api","village_api","location_statistics", "summary_governorate","get_data_map"]):
			return True
			...
			return True
		elif(type(request.user)!=AnonymousUser and request.user.groups.all().filter(name="farmer").count()>0 and  view.__class__.__name__ in ["img_farmer_api","farm_info_list","farm_info","farm_platoon_api", "search_farm_api","farm_species_api","get_animal_farm"]):
			return True
